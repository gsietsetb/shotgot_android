...
import com.shotgot.shotgot.utils.AutoFitTextureView;
...
public class ShotFragment extends ImmersiveModeFragment
        implements View.OnClickListener, FragmentCompat.OnRequestPermissionsResultCallback {// implements CameraBridgeViewBase.CvCameraViewListener2 {
                        ...
                        switch ((String) respTag.get("type")) {
                            ...
                            case "TAGs":
                                ...
                                for (int i = 0; i < tags.length(); i++) {
                                    addRespLabels((String) tags.get(i));
                                }
                                break;
                            case "OCR":
                                String ocr = (String) respTag.get("data");
                                if (ocr != null)
                                    addRespText(ocr);
                                break;
                            default:
                                String data = (String) respTag.get("data");
                                if (data != null)
                                    addRespLabels(data);
                        }
...
//                    final Gson gson = new GsonBuilder().serializeNulls().create();
                    ...
                    /** Simple Method*/
//                    MetaModel tagRx = gson.fromJson(respTag, MetaModel.class);

                    /** Multi-type Method*/
//                    MetaModel collectionType = (MetaModel) new TypeToken<Collection<MetaModel>>(){}.getType();
//                    Collection<MetaModel> tags = gson.fromJson(respTag, (Type) collectionType);
...
//                    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_shot2, container, false);
    }
...
//                ErrorDialog.newInstance("Sorry you need further Camera permissions")
//                        .show(getChildFragmentManager(), FRAGMENT_DIALOG);
                Log.e("CAMEAR ERROR", "PERMISION ERROR DUDE");
            }
        } else {
...
//            ErrorDialog.newInstance("CAMERA ERROR, please dude!")
//                    .show(getChildFragmentManager(), FRAGMENT_DIALOG);
            Log.d("CAMERA_ERROR", "There is a fucking errror with the camera!");
        }
    }

    /**
     * Opens the camera specified by {@link ShotFragment#mCameraId}.
     */
    private void openCamera(int width, int height) {
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            Log.d("CAMERA", "Should be requesting permisions");
...
//        }
    ...
    private void addRespText(String tag) {
        TextView tagView = new TextView(getContext());
        tagView.setTextColor(getResources().getColor(R.color.wallet_bright_foreground_holo_dark));
        Log.d("AddView", rand.nextInt(580) + " is the place to add " + tag);
        tagView.setText(tag);
    ...
    public static class ConfirmationDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            return new AlertDialog.Builder(getActivity())
                    .setMessage("I told u about permissions!")
...
//            mFile = file;
        ...
        public void run() {
            ...
            Log.d("CAMERA", "sending img " + mImage.getFormat() + buffer.toString());
            byte[] data = new byte[buffer.remaining()];
            buffer.get(data);
            Log.d("CAMERA", "sending data " + data);

            mSocket.connect();
            String imgString = Base64.encodeToString(data, Base64.NO_WRAP);
            Log.d("CAMERA", "sending b64. " + imgString);
            mSocket.emit("PIC_REQ", imgString);
//            buffer.get(bytes);
